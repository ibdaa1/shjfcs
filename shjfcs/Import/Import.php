...
// آمن: سجل الأخطاء داخل ملف لكن لا تعرض للمستخدم
...
try {
    // تحميل إعدادات الاتصال - يتوقع ملف ../db.php أن يعرف $conn كـ mysqli
    $dbFile = realpath(__DIR__ . '/../db.php');
    if (!$dbFile || !file_exists($dbFile)) {
        throw new RuntimeException("Database configuration file '../db.php' not found.");
    }
    require_once $dbFile;
    if (!isset($conn) || !($conn instanceof mysqli)) {
        throw new RuntimeException("Database connection object \$conn is not available. Check db.php.");
    }

    if (session_status() === PHP_SESSION_NONE) session_start();

    // تحقق من جلسة المستخدم - يمكن تعديل هذا السلوك إذا أردت السماح بالوصول دون تسجيل
    ...
    // قراءة JSON body helper
    function json_input() {
        $raw = file_get_contents('php://input');
        $data = json_decode($raw, true);
        if ($data === null) return $_POST;
        return $data;
    }

    // تحقق أن unique_id ينتمي لlicense_no طبقاً لجدول establishments
    function validate_unique_belongs_to_license($conn, $unique_id, $license_no) {
        if (!$unique_id || !$license_no) return false;
        $stmt = $conn->prepare("SELECT ID FROM establishments WHERE unique_id = ? AND license_no = ? LIMIT 1");
        $stmt->bind_param('ss', $unique_id, $license_no);
        $stmt->execute();
        $res = $stmt->get_result();
    ...
    switch ($action) {
        case 'list':
            $sql = "SELECT
                        ie.ID, ie.unique_id, ie.registration_date, ie.system_registration_no, ie.container_count, ie.container_numbers,
                        ie.actual_inspection_date,
                        p.port_name AS port_name,
                        u.EmpName AS inspector_name,
                        a.action_name AS action_name,
                        ie.notes, ie.created_at
        ...
        case 'create':
            ...
            if (!validate_unique_belongs_to_license($conn, $unique_id, $license_no)) {
                http_response_code(400);
                echo json_encode(['error'=>'المعرف الفريد لا ينتمي إلى رقم الرخصة المحدد']);
                exit;
            }

            // تحويل تنسيقات التاريخ: تحويل T --> space إن أرسلها العميل من datetime-local
            ...
            $entry_port_id = $input['entry_port_id'] ?: null;
            $system_registration_no = $input['system_registration_no'] ?: null;
            $container_count = $input['container_count'] !== '' ? $input['container_count'] : null;
            $container_numbers = $input['container_numbers'] ?: null;
            $inspector_emp_id = $input['inspector_emp_id'] ?: null;
            $action_taken_id = $input['action_taken_id'] ?: null;
            $notes = $input['notes'] ?: null;
            $created_by_emp_id = $empid ?: null;

            // bind with appropriate types (s = string, i = integer)
            $stmt->bind_param('ssississsis',
                $unique_id,
                $reg_date,
                $entry_port_id,
                $system_registration_no,
                $container_count,
                $container_numbers,
                $actual_date,
                $inspector_emp_id,
            ...
            if (!empty($products)) {
                ...
                foreach ($products as $p) {
                    ...
                    $serial = $p['serial'] ?? '';
                    $notes_p = $p['notes'] ?? '';
                    $pstmt->bind_param('ississddssss',
                        $inspection_id, $product_name, $brand_name, $country_id, $packaging_type, $quantity, $weight, $production_date, $expiry_date, $category_id, $serial, $notes_p
                    );
                    $pstmt->execute();
                }
                $pstmt->close();
        ...
        case 'update':
            if (!$isAdmin && !$canEdit) { http_response_code(403); echo json_encode(['error'=>'Permission denied']); exit; }
            $input = json_input();
        ...
        case 'delete':
            if (!$isAdmin && !$canDelete) { http_response_code(403); echo json_encode(['error'=>'Permission denied']); exit; }
            $id = intval($_GET['id'] ?? 0);
            if (!$id) { http_response_code(400); echo json_encode(['error'=>'id required']); exit; }
    ...
    // سجل الخطأ للمراجعة، وأعد رسالة عامة
